...
typedef struct NODE
{
	BYTE2 symbol;BYTE4 usage;BYTE2 count;BYTE2 branch;
	struct NODE **tree;
} TREE;
...
static const char *errorfilename = "megahal.log";
static const char *statusfilename = "megahal.txt";
...
static std::ostream* errorfp;
static std::ostream* statusfp;
...
#define error(title, msg) \
	do \
	{ \
		if(errorfp) (*errorfp) << title << ": " << msg << ".\n"; \
		if(errorfp) (*errorfp) << "MegaHAL died for some reason; check the error log.\n"; \
	} \
	while(false)

#define warn(title, msg) \
do \
{ \
	if(errorfp) (*errorfp) << title << ": " << msg << ".\n"; \
	if(errorfp) (*errorfp) << "MegaHAL emitted a warning; check the error log.\n"; \
} while(false)

#define status(msg) \
do \
{ \
	if(statusfp) (*statusfp) << msg << std::flush; \
} while(false)
...
static bool initialize_error(const char *);
static bool initialize_status(const char *);
...
static void write_output(char *, std::ostream& os);
...
static bool print_header(std::ostream&);
...
char *megahal_do_reply(const char *input, int /* log */)
{
	char *output = NULL;
...
void megahal_output(char *output, std::ostream& os)
{
	if (!quiet)
		write_output(output, os);
}
...
int megahal_command(char *input, std::ostream& os)
{
	int position = 0;
	char *output;
 ...
 */
bool initialize_error(const char *filename)
{
...
//  errorfp = fopen(filename, "a");
	errorfp = new std::ofstream(filename, std::ios::app);
	if(errorfp == NULL)
	{
//	errorfp=stderr;
		errorfp = &std::cerr;
 ...
 */
bool initialize_status(const char *filename)
{
//	if (statusfp != ::stdout)
//		::fclose(statusfp);
	if (statusfp != &std::cout)	{ delete statusfp; statusfp = 0; }
	if (filename == NULL) return false;

//	statusfp = fopen(filename, "a");
	statusfp = new std::ofstream(filename, std::ios::app);
	if(statusfp == NULL)
	{
//		statusfp = stdout;
		statusfp = &std::cout;
...
bool print_header(std::ostream& file)
{
	time_t clock;
	char timestamp[1024];
	struct tm *local;
	...
	file << "MegaHALv8\n";
	file << "Copyright (C) 1998 Jason Hutchens\n";
	file << timestamp;
	file << std::flush;
 ...
 */
void write_output(char *output, std::ostream& os)
{
	char *formatted;
	char *bit;
...
//	delay(formatted);
	...
	if (bit == NULL) status("MegaHAL: " << formatted << "\n");
	while (bit != NULL)
	{
		status("MegaHAL: " << bit << '\n');
		bit = strtok(NULL, "\n");
...
void write_input(char *input)
{
	...
	bit = strtok(formatted, "\n");
	if (bit == NULL) status("User:    " << formatted << '\n');
	while (bit != NULL)
	{
		status("User:    " << bit << '\n');
		bit = strtok(NULL, "\n");
...
static char *format_output(char *output)
{
	...
	formatted = (char *) realloc((char *) formatted,
		sizeof(char) * (strlen(output) + 2));
	if (formatted == NULL)
	{
 ...
 */BYTE2 add_word(DICTIONARY *dictionary, STRING word)
{
	...
	{
		dictionary->index = (BYTE2 *) malloc(
			sizeof(BYTE2) * (dictionary->size));
	}
	else
	{
		dictionary->index = (BYTE2 *) realloc((BYTE2 *) (dictionary->index),
			sizeof(BYTE2) * (dictionary->size));
	}
	if (dictionary->index == NULL)
	{
	...
	{
		dictionary->entry = (STRING *) malloc(
	...
	if (dictionary->entry == NULL)
	{
		error("add_word", "Unable to reallocate the dictionary to " << dictionary->size << " elements.");
		goto fail;
	}
	...
	dictionary->entry[dictionary->size - 1].word = (char *) malloc(
		sizeof(char) * (word.length));
	...
	succeed: return (dictionary->index[position]);
...
void update_model(MODEL *model, int symbol)
{
	...
	for (i = (model->order + 1); i > 0; --i)
		if (model->context[i - 1] != NULL
			...
			model->context[i] = add_symbol(model->context[i - 1],
				(BYTE2) symbol);
...
void add_node(TREE *tree, TREE *node, int position)
{
	...
	{
		tree->tree = (TREE **) realloc((TREE **) (tree->tree),
			sizeof(TREE *) * (tree->branch + 1));
	}
	if (tree->tree == NULL)
	{
...
int search_node(TREE *node, int symbol, bool *found_symbol)
{
	...
	found: *found_symbol = true;
	return (position);

	notfound: *found_symbol = false;
	return (position);
}
...
void save_model(char *modelname, MODEL *model)
{
	...
	if (file == NULL)
	{
		warn("save_model", "Unable to open file `" << filename << "'");
		return;
	}
...
bool load_model(char *filename, MODEL *model)
{
	...
	if (file == NULL)
	{
		warn("load_model", "Unable to open file `" << filename << "'");
		return (false);
	}
	...
	{
		warn("load_model", "File `" << filename << "' is not a MegaHAL brain");
		goto fail;
	}
...
void make_words(char *input, DICTIONARY *words)
{
	...
	while (1)
	{
		...
		if (boundary(input, offset))
		{
			...
			if (words->entry == NULL
				...
				words->entry = (STRING *) malloc(
					(words->size + 1) * sizeof(STRING));
			else
				words->entry = (STRING *) realloc(words->entry,
					(words->size + 1) * sizeof(STRING));
	...
	if (isalnum(words->entry[words->size - 1].word[0]))
	{
		...
		"!.?",
		words->entry[words->size - 1].word[words->entry[words->size - 1].length
			- 1]) == NULL)
	{
...
void make_greeting(DICTIONARY *words)
{
	...
	free_dictionary(words);
	if (grt->size > 0)
		(void) add_word(words, grt->entry[rnd(grt->size)]);
}
...
char *generate_reply(MODEL *model, DICTIONARY *words)
{
	...
	do
	{
		...
		if ((surprise > max_surprise)
			&& (dissimilar(words, replywords) == true))
		{
...
DICTIONARY *reply(MODEL *model, DICTIONARY *keys)
{
	...
	while (true)
	{
		...
		if (replies->entry == NULL
			...
			replies->entry = (STRING *) malloc(
				(replies->size + 1) * sizeof(STRING));
		else
			replies->entry = (STRING *) realloc(replies->entry,
				(replies->size + 1) * sizeof(STRING));
		if (replies->entry == NULL)
		{
...
int seed(MODEL *model, DICTIONARY *keys)
{
	...
	if (model->context[0]->branch == 0)
		...
		symbol =
			model->context[0]->tree[rnd(model->context[0]->branch)]->symbol;
...
{
	if (sig != 0)
		warn("ignore", "MegaHAL received signal " << sig);
...
//void die(int sig)
//{
//	error("die", "MegaHAL received signal " << sig);
//	exithal();
//}
...
//	 *    needs to be done.
//	 */
//	if ((done * 100 / total == 100) && (first == false))
//		return (true);
//
//	/*
...
//
//	if (done > 0)
//		fprintf(stderr, "%c%c%c%c", 8, 8, 8, 8);
...
void load_personality(MODEL **model)
{
	...
	if (strcmp(directory, last) != 0)
	{
		...
		if (file == NULL)
		{
			...
			{
				fprintf(stdout,
					"Unable to change MegaHAL personality to \"%s\".\n"
...
void change_personality(DICTIONARY *command, unsigned int position,
	MODEL **model)
{
